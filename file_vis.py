...
@st.cache_data
def render_pdf_pages(filename: str, dpi: int = 82) -> list[str]:
    """
    Convert each page of a PDF from wwwroot/my_pages to a base64-encoded PNG.
    """
    try:
        ...
        st.error(f"Error rendering PDF {filename}: {e}")
        return []

def display_pdf(well_list: list[str], files_df: pd.DataFrame):
    """
    For each well in well_list, look up its PDF filename in files_df and render from wwwroot/my_pages.
    """
    for well in well_list:
        ...
        if not pdf_row.empty:
            filename = pdf_row.iloc[0]['file_path']  # e.g., document.pdf
            st.write(f"**{well}** ‣ `{filename}`")
            pages = render_pdf_pages(filename)
            if pages:
                imgs = "".join(
                ...
                st.error(f"Could not load PDF for {filename}.")
        else:
